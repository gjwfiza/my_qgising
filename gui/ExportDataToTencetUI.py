...
from MyQGIS.config.HeadsConfig import Tencent_SiteFields, Tencent_CellFields
from MyQGIS.Contorls.ExportDataToTencent import exportDataToTencent

#
class ExportDataToTencentUI(QtGui.QDialog):
    def __init__(self, iface, parent=None):
        super(ExportDataToTencentUI, self).__init__()
        self.iface = iface
        self.parent = parent
    ...
    def initUI(self):
        ...
        self.label1 = QtGui.QLabel(u"<b>请输入您的腾讯地图密钥： </b>")
        self.key_edit = QtGui.QLineEdit(u"4V3BZ-SOKHU-EVAVO-4XNJR-GNNFQ-MXF3M") # my Key
        # self.key_edit = QtGui.QLineEdit(u"IPKBZ-S4465-O5RIK-QLZFV-OWEXV-LUBF4") # 德政key
        ...
        self.cell_button = QtGui.QRadioButton(u' 小区(目前仅支持少量数据) ')
        ...
        self.link_label = QtGui.QLabel(u"腾讯地图秘钥申请链接: http://lbs.qq.com/mykey.html ")
    ...
    def export(self):
        ...
        if self.key != None and self.key != u"":
            ...
            if self.site_button.isChecked():
                self.type = TencentType.Site
            else:
                self.type = TencentType.Cell

            filedir = QtGui.QFileDialog.getExistingDirectory(None, 'Export ', '/')
            if filedir != None and filedir != '':
                datas = self.getDatas()
                result = exportDataToTencent(filedir, self.key, self.type, datas, self.parent)
                if result == True:
    ...
    # 将api申请链接复制到剪贴板
    def copyLink(self):
        apply_link = "http://lbs.qq.com/mykey.html"
        w.OpenClipboard()
        w.EmptyClipboard()
        ...
        if self.type == TencentType.Site:
            ...
            for site in outputSites:
                ...
                if site[u'运营商'] in operator_list:
                    temp = []
                    for field in Tencent_SiteFields:
            ...
            for cell in outputCells:
                ...
                if cell[u'运营商'] in operator_list:
                    temp = []
                    for field in Tencent_CellFields:
                        if cell[field] != None:
