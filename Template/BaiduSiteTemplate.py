...
# 百度地图基站数据html模板
class BaiduSiteTemplate(object):
    def __init__(self, ak=u"BtvVWRqGfnffNqdqAN7rusTlb2E020C8"):
        super(BaiduSiteTemplate, self).__init__()
        self.ak = ak
                ...
                */
                if (document.createElement('canvas').getContext) {  // 判断当前浏览器是否支持绘制海量点
                    ...
                    for (var i = 0; i < site.length; i++) {
                        var temp = new BMap.Point(site[i].bd_lon, site[i].bd_lat)
                        temp.index = i
                        points.push(temp);
                    }
                    ...
                    var SiteName = site[i].SiteName;
                    var SiteId = site[i].SiteId;
                    var addr = String(site[i].lon)+","+String(site[i].lat);
                    ...
                    var infowindow = new BMap.InfoWindow("基站名称:"+SiteName+"<br/>基站ID:"+SiteId+"<br/>经纬度:"+addr, opts);
                ...
                function searchName(){
                    ...
                    for (var i = 0; i < site.length; i++){
                        if (keyword == (site[i].SiteName))
                        {
                            ...
                            var point = new BMap.Point(site[i].bd_lon, site[i].bd_lat);
                ...
                function LabelOpen(){
                    for (var i=0; i<site.length; i++)
                    {
                        ...
                        var label = new BMap.Label(site[i].SiteName, label_opts);  // 创建文本标注对象
            ...
            <script type="text/javascript">
                        ...
                        convertor.translate(pointArr, 1, 5, function(data){
                            if(data.status === 0)
                            {
                                ...
                                if (site_name != "")
                                {
                                    ...
                                    var infoWindow = new BMap.InfoWindow(site_name, opts);  // 创建信息窗口对象
                ...
                map.addEventListener("click",function(e){
                    if (marker != null)
                    {
                        ...
                        // 计算街景位置与基站距离
                        document.getElementById("distance").innerHTML ="街景位置与基站距离"+map.getDistance(site_point,pano_point).toFixed(2)+"米";
                        polyline = new BMap.Polyline([site_point,pano_point], {strokeColor:"blue", strokeWeight:6, strokeOpacity:0.5});  //定义折线
