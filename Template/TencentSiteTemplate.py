...
# 腾讯地图基站数据html模板
class TencentSiteTemplate(object):
    def __init__(self, key=u"G2TBZ-VWXR5-XKRIU-QOTEH-LWV4Z-R3FSW"):
        super(TencentSiteTemplate, self).__init__()
        self.key = key
    ...
    def getHead(self):
        output = u"""
            ...
            <script type="text/javascript">
                ...
                {
                    (function(n){
                        var position = new qq.maps.LatLng(site[n].qq_lat,site[n].qq_lon)
                        var point = new qq.maps.Marker({
                            position: position,
                            map: map
                        });
                        var label = new qq.maps.Label ({
                            ...
                            map: map,
                            content: site[n].SiteName,
                            visible: false // 默认标签不可见
                        });
                        labelArry.push(label);
                        qq.maps.event.addListener(point, 'click', function() {
                            infoWin.open();
                            infoWin.setContent("基站名称:"+site[n].SiteName+"<br/>基站ID:"+site[n].SiteId+"<br/>经纬度:"+site[n].lon +","+site[n].lat);
                            infoWin.setPosition(new qq.maps.LatLng(site[n].qq_lat,site[n].qq_lon));
                        });
                    })(i);
                }
                // 查找功能
                        ...
                        if (keyword == (site[i].SiteName))
                        {
                            ...
                            */
                            var marker = new qq.maps.Marker({
                                //icon: icon,
                                position: new qq.maps.LatLng(site[i].qq_lat,site[i].qq_lon),
                                map: map
                            });
                            //marker.setIcon(icon);
                            infoWin.open();
                            infoWin.setContent("基站名称:"+site[i].SiteName+"<br/>基站ID:"+site[i].SiteId+"<br/>经纬度:"+site[i].lon +","+site[i].lat);
                            infoWin.setPosition(new qq.maps.LatLng(site[i].qq_lat,site[i].qq_lon));

                            markersArray.push(marker);
                            map.panTo(new qq.maps.LatLng(site[i].qq_lat,site[i].qq_lon));
                            map.zoomTo(18);
                        }
                    }
                    if (result == false)
        ...
        output = u"""
            ...
            <script type="text/javascript">
                    ...
                    if (document.getElementById("SearchName").checked == true) // 按基站名称搜索
                    {
                        ...
                        for (var i = 0; i < site.length; i++){
                            if (keyword == (site[i].SiteName))
                            {
                                ...
                                var point = new qq.maps.LatLng(site[i].qq_lat, site[i].qq_lon);
                                var marker = new qq.maps.Marker({
                                    position: point,
                                    map: map
                                });
                ...
                qq.maps.event.addListener(map,"click",function(e){
                    if ((markersArray.length != 0) && (site_point != null))
                    {
                        ...
                        // 计算街景位置与基站夹角
                        if ((pano_point.lng>site_point.lng)&&(pano_point.lat>site_point.lat))
                        {
                            angle = Math.atan((pano_point.lng - site_point.lng) / (pano_point.lat - site_point.lat));
                            document.getElementById("angle").innerHTML ="街景位置与基站夹角"+Math.abs(angle)/Math.PI*180+"度";
                        }
                        else if ((pano_point.lng>site_point.lng)&&(pano_point.lat<site_point.lat))
                        {
                            angle = Math.atan(Math.abs(pano_point.lng - site_point.lng) / Math.abs(pano_point.lat - site_point.lat));
                        ...
                        // 计算街景位置与基站距离
                        document.getElementById("distance").innerHTML ="街景位置与基站距离"+qq.maps.geometry.spherical.computeDistanceBetween(site_point,pano_point).toFixed(2)+"米";
                        //添加连线
                        var line_path = [site_point, pano_point];
